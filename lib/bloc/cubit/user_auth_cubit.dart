...
import '../states/user_auth_state.dart';
...
class UserAuthCubit extends Cubit<UserAuthState> {
  ...
  Future<String?> _getFaviconUrl(bool popularAuth, String authName) async {
    try {
      ...
      // Check external favicon links first.
      final faviconLinks = [
      ...
      // If external links fail, check Firebase Storage for a favicon whose name
      // starts with the first 4 characters of the cleaned auth name.
      if (cleanAuthName.length >= 4) {
        final prefix = cleanAuthName.substring(0, 4).toLowerCase();
        final storageRef = FirebaseStorage.instance.ref('favicons');
